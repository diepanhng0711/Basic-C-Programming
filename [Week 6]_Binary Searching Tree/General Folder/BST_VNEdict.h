...
//Hàm search tiếng Anh
TreeType Search_Eng(char* name, TreeType Tree) {
    if (Tree == NULL) return NULL;
    else if (strcmp((Tree->key).eng, name) == 0) return Tree;
    else if (strcmp((Tree->key).eng, name) < 0) return Search_Eng(name, Tree->right);
    else return Search_Eng(name, Tree->left);
}

//Hàm search tiếng Việt
TreeType Search_Vie(char* name, TreeType Tree) {
    if (Tree == NULL) return NULL;
    else if (strcmp((Tree->key).vie, name) == 0) return Tree;
    else if (strcmp((Tree->key).vie, name) < 0) return Search_Vie(name, Tree->right);
    else return Search_Eng(name, Tree->left);
}

//Hàm insert 1 node bất kỳ vào cây
//Lưu ý: Không tồn tại 2 nodes có key giống nhau trong cây tìm kiếm nhị phân
void Insert_Node_Vie(KeyType data, TreeType *Tree) {
    if (*Tree == NULL) {
       *Tree = create_Node(data);
    } else if (strcmp(((*Tree)->key).vie, data.vie) < 0) Insert_Node_Vie(data, &(*Tree)->right);
    else Insert_Node_Vie(data, &(*Tree)->left);
}

void Insert_Node_Eng(KeyType data, TreeType *Tree) {
    if (*Tree == NULL) {
       *Tree = create_Node(data);
    } else if (strcmp(((*Tree)->key).eng, data.eng) < 0) Insert_Node_Eng(data, &(*Tree)->right);
    else Insert_Node_Eng(data, &(*Tree)->left);
}
